 ...
 */
class SalaryItemsAddition extends \yii\db\ActiveRecord
{
    use CoreTrait;
    ...
    public function beforeValidate()
    {
        if (in_array($this->getScenario(), [self::SCENARIO_CREATE_LOW_TIME, self::SCENARIO_CREATE_OVER_TIME, self::SCENARIO_CREATE_COMMISSION, self::SCENARIO_CREATE_NON_CASH])) {
            $this->from_date = strtotime(Jdf::Convert_jalali_to_gregorian($this->date) . " 00:00:00");
            ...
            $this->from_date = strtotime(Jdf::Convert_jalali_to_gregorian($this->from_date) . " 00:00:00");
            $this->to_date = strtotime(Jdf::Convert_jalali_to_gregorian($this->to_date) . " 00:00:00");
        }
    ...
    {
        $title = SalaryItemsAddition::itemAlias('Kind', $this->kind) . ' - ' . SalaryItemsAddition::itemAlias('Type', $this->type) . ' - ' . $this->getValue() . ' - ';
        if ($this->kind == SalaryItemsAddition::KIND_COMMISSION || SalaryItemsAddition::KIND_NON_CASH || $this->kind == SalaryItemsAddition::KIND_COMMISSION_CONST) {
    ...
    public function beforeSave($insert)
    {
        if ($this->isNewRecord) {
            ...
            if ($this->getScenario() == self::SCENARIO_CREATE_AUTO) {
                $this->is_auto = Helper::CHECKED;
            } else {
                $this->is_auto = Helper::UN_CHECKED;
                $this->status = self::STATUS_WAIT_CONFIRM;
            }
        }
