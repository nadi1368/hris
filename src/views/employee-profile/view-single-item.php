        ...
        <table class='table table-bordered' style="table-layout: fixed">
            ...
            <?php if ($model->commission > 0): ?>
                <tr>
                    ...
                    <td colspan='4'>
                        <?php
                        if (isset($model->detailAddition['commission']) && is_array($model->detailAddition['commission'])) {
                            foreach ($model->detailAddition['commission'] as $item) {
            ...
            <?php if ($model->hours_of_overtime > 0): ?>
                <tr>
                    ...
                    <td colspan='4'>
                        <?php
                        foreach (SalaryItemsAddition::find()->bySalary($model->user_id, SalaryItemsAddition::KIND_OVER_TIME, SalaryItemsAddition::TYPE_OVER_TIME_DAY, $model->period->start_date, $model->period->end_date)->all() as $item) {
            ...
            <?php if ($model->holiday_of_overtime > 0): ?>
                <tr>
                    ...
                    <td colspan='4'>
                        <?php
                        foreach (SalaryItemsAddition::find()->bySalary($model->user_id, SalaryItemsAddition::KIND_OVER_TIME, SalaryItemsAddition::TYPE_OVER_TIME_HOLIDAY, $model->period->start_date, $model->period->end_date)->all() as $item) {
                            echo Html::tag('p', $item->title);
                        }
                        ?>
                    </td>
            ...
            <?php if ($model->night_of_overtime > 0): ?>
                <tr>
                    ...
                    <td colspan='4'>
                        <?php
                        foreach (SalaryItemsAddition::find()->bySalary($model->user_id, SalaryItemsAddition::KIND_OVER_TIME, SalaryItemsAddition::TYPE_OVER_TIME_NIGHT, $model->period->start_date, $model->period->end_date)->all() as $item) {
                            echo Html::tag('p', $item->title);
                        }
                        ?>
                    </td>
            ...
            <?php if ($model->hoursOfLowTime > 0): ?>
                <tr>
                    <td colspan='4'>
                        <?php
                        foreach (SalaryItemsAddition::find()->bySalary($model->user_id, SalaryItemsAddition::KIND_LOW_TIME, null, $model->period->start_date, $model->period->end_date)->all() as $item) {
                            echo Html::tag('p', $item->title);
                        }
                        ?>
                    </td>
            ...
            <tr>
                <td colspan='2'>مبلغ خالص پرداختی</td>
