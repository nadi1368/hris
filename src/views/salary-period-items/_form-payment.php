...
<div class="card">
    <div class="card-body">
        <?php foreach ($model->getSalaryPeriodItems()->all() as $index => $item): ?>
            <?php if (($amount = (Module::getInstance()->balanceDetailedClass::getBalance(Module::getInstance()->settings::get('salary_period_payment_m_id', true), $item->employee->account_id) * (-1))) !== $item->finalPayment): ?>
                <?php
                $showUpdateBtn = true;
