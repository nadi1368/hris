 ...
 */
class YearPeriodItemsBase extends Controller
{
    ...
    public function actionReturnConfirm($id)
    {
        ...
        if ($model->canReturnConfirm()) {
            ...
            try {
                ...
                $flag = $model->save(false);
                $flag = $flag && $model->deleteDocument(SalaryPeriod::DOCUMENT_TYPE_YEAR_PERIOD_CLEARING);
                $flag = $flag && $model->deleteDocument(SalaryPeriod::DOCUMENT_TYPE_YEAR_PERIOD);
                if ($flag) {
                    $transaction->commit();
                    $result = [
                        'status' => true,
